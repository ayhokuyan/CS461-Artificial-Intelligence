...
driver.get('https://www.nytimes.com/crosswords/game/special/pets-mini')
time.sleep(0.2)
driver.find_element_by_xpath(
    "/html/body/div[1]/div/div/div[4]/div/main/div[2]/div/div[2]/div[3]/div/article/div[2]/button/div/span").click()
time.sleep(0.2)
driver.find_element_by_xpath("/html/body/div[1]/div/div/div[4]/div/main/div[2]/div/div/ul/div[2]/li[2]/button").click()
time.sleep(0.2)
driver.find_element_by_xpath(
    "/html/body/div[1]/div/div/div[4]/div/main/div[2]/div/div/ul/div[2]/li[2]/ul/li[3]/a").click()
time.sleep(0.2)
...
for word in acrossWords:
    clueDict[word] = []
    driver.get('https://www.oxfordlearnersdictionaries.com/definition/english/' + str.lower(word) + '?q=' + str.lower(word))
    for i in range(1):
        try:
            temp = driver.find_element_by_xpath(
                '//*[@id="' + str.lower(word) + '_def_1"]').get_property(
                'textContent')
        except:
            break
...
for word in downWords:
    clueDict[word] = []
    driver.get(
        'https://www.oxfordlearnersdictionaries.com/definition/english/' + str.lower(word) + '?q=' + str.lower(word))
    for i in range(1):
        try:
            temp = driver.find_element_by_xpath(
                '//*[@id="' + str.lower(word) + '_def_1"]').get_property(
...
for word in acrossWords:
    driver.get('https://www.merriam-webster.com/dictionary/' + str.lower(word))
    for i in range(1):
        try:
            temp = driver.find_element_by_xpath(
                '//*[@id="dictionary-entry-1"]/div[2]/div[1]/span[1]/div/span[2]/span').get_property(
                'textContent')
        except:
            break
        temp = temp.capitalize()
...
for word in acrossWords:
    ...
    for i in range(1):
        try:
            temp = driver.find_element_by_xpath(
                '/html/body/div[1]/div/div/div[2]/div/main/section/section/div[1]/section[2]/div/div').get_property(
                'textContent')
            if len(temp) < 90:
                temp = temp.capitalize()
                clueDict[word].append(temp)
...
for word in acrossWords:
    ...
    for i in range(1, 5):
        if i != 2:
            try:
                temp = driver.find_element_by_xpath(
                    '/html/body/div[2]/div[4]/div/div/div[1]/div[1]/div[' + str(i) + ']/div[3]').get_property(
                    'textContent')
                temp = fix_data(temp)
                for val in temp:
                    if len(val) < 90:
    ...
    for j in range(5):
        ...
        if reveals[x][0] == "black":
            ...
            canvas.create_text((100 * (j + 1)) + 50, (100 * (i + 1)) + 50, fill="blue", font="Arial 40 bold",
                               text=reveals[x][1][0])
            canvas.create_text((100 * (j + 1)) + 10, (100 * (i + 1)) + 10, fill="black", font="Arial 15 ",
                               text=reveals[x][2])
        else:
            canvas.create_rectangle(100 * (j + 1), 100 * (i + 1), 100 + (100 * (j + 1)), 100 + (100 * (i + 1)),
                                    fill="white")
...
cnt2 = 0
for i in range(len(clues_of_across)):
    org_clue =  clues_of_across[i][2]
    if len(org_clue)>120:
        org_clue_div = org_clue[0:120].rsplit(" ",1)
        org_clue_div[1] = org_clue[len(org_clue_div[0])+1:]
        print(org_clue_div)
        print(org_clue_div[0])
        print(org_clue_div[1])
        canvas.create_text(640, 170 + (i*40 + cnt*20), fill="gray", font="Arial 13", text=clues_of_across[i][1] + " " + org_clue_div[0] , anchor="w")
        cnt+=1
        canvas.create_text(640, 170 + (i*40 + cnt*20), fill="gray", font="Arial 13", text="   " + org_clue_div[1] , anchor="w")
    else:
        canvas.create_text(640, 170 + (i*40 + cnt*20), fill="gray", font="Arial 13", text=clues_of_across[i][1] + " " + org_clue , anchor="w")

    gen_clue = (clueDict[acrossWords[i]][0])
    gen_clue = gen_clue.split('.',1)[0]
    gen_clue = gen_clue.split(':',1)[0]

    if len(gen_clue)>120:
        gen_clue_div = gen_clue[0:120].rsplit(" ",1)
        gen_clue_div[1] = gen_clue[len(gen_clue_div[0])+1:]
        print(gen_clue_div)
        print(gen_clue_div[0])
        print(gen_clue_div[1])
        canvas.create_text(640, 170 + (i*40 + cnt*20+20), fill="purple", font="Arial 13", text=clues_of_across[i][1] + " " + gen_clue_div[0] , anchor="w")
        cnt+=1
        canvas.create_text(640, 170 + (i*40 + cnt*20+20), fill="purple", font="Arial 13", text="   " + gen_clue_div[1] , anchor="w")
    else:
        canvas.create_text(640, 170 + (i*40 + cnt*20+20), fill="purple", font="Arial 13", text=clues_of_across[i][1] + " " + gen_clue , anchor="w")

canvas.create_text(680, 200 + (len(clues_of_across))*40, fill="black", font="Arial 20 bold", text="DOWN")

for i in range(len(clues_of_down)):
    org_clue =  clues_of_down[i][2]
    if len(org_clue)>120:
        org_clue_div = org_clue[0:120].rsplit(" ",1)
        org_clue_div[1] = org_clue[len(org_clue_div[0])+1:]
        print(org_clue_div)
        print(org_clue_div[0])
        print(org_clue_div[1])
        canvas.create_text(640, 50 + (len(clues_of_across)+4)*40 + (i*40 + cnt*20), fill="gray", font="Arial 13", text=clues_of_down[i][1] + " " + org_clue_div[0] , anchor="w")
        cnt+=1
        canvas.create_text(640, 60 + (len(clues_of_across)+4)*40 + (i*40 + cnt*20), fill="gray", font="Arial 13", text="   " + org_clue_div[1] , anchor="w")
    else:
        canvas.create_text(640, 60 + (len(clues_of_across)+4)*40 + (i*40 + cnt*20), fill="gray", font="Arial 13", text=clues_of_down[i][1] + " " + org_clue , anchor="w")

    gen_clue = (clueDict[downWords[i]][0])
    gen_clue = gen_clue.split('.',1)[0]
    gen_clue = gen_clue.split(':',1)[0]
    if len(gen_clue)>120:
        gen_clue_div = gen_clue[0:120].rsplit(" ",1)
        ...
        print(gen_clue_div[0])
        print(gen_clue_div[1])
        canvas.create_text(640, 60 + (len(clues_of_across)+4)*40 + (i*40 + cnt*20+20), fill="purple", font="Arial 13", text=clues_of_down[i][1] + " " + gen_clue_div[0] , anchor="w")
        cnt+=1
        canvas.create_text(640, 60 + (len(clues_of_across)+4)*40 + (i*40 + cnt*20+20), fill="purple", font="Arial 13", text="   " + gen_clue_div[1] , anchor="w")
    else:
        canvas.create_text(640, 60 + (len(clues_of_across)+4)*40 + (i*40 + cnt*20+20), fill="purple", font="Arial 13", text=clues_of_down[i][1] + " " + gen_clue , anchor="w")


root.mainloop()
'''
canvas.create_text(640, 170 + (i * 40), fill="gray", font="Arial 15",
                       text=clues_of_across[i][1] + " " + clues_of_across[i][2], anchor="w")
    canvas.create_text(640, 170 + (i * 40) + 20, fill="purple", font="Arial 15",
                       text=clues_of_across[i][1] + " " + (clueDict[acrossWords[i]][0]), anchor="w")

canvas.create_text(680, 170 + (len(clues_of_across) + 2) * 40, fill="black", font="Arial 20 bold", text="DOWN")
for i in range(len(clues_of_down)):
    canvas.create_text(640, 130 + (len(clues_of_across) + 4) * 40 + (i * 40), fill="gray", font="Arial 15",
                       text=clues_of_down[i][1] + " " + clues_of_down[i][2], anchor="w")
    canvas.create_text(640, 130 + (len(clues_of_across) + 4) * 40 + (i * 40) + 20, fill="purple", font="Arial 15",
                       text=clues_of_down[i][1] + " " + (clueDict[downWords[i]][0]), anchor="w")
